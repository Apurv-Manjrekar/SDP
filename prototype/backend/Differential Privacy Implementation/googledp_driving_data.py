...
def apply_differential_privacy(df, numeric_columns, epsilon=5):
    """
    ...
    for column in numeric_columns:
        if column in df_copy.columns:
            ...
            df_copy[f"{column}_DP"] = df_copy[column].apply(lambda x: add_laplace_noise(x, sensitivity, epsilon))
...
if __name__ == "__main__":
    ...
    numeric_columns = ["Speed", "Acceleration", "Time", "Vehicle ID", "Latitude", 
                       "Longitude", "Time Gap", "Headway", "Distance", "Lane Changes"]

    # Apply Differential Privacy
    df_dp = apply_differential_privacy(df, numeric_columns, epsilon=5)

    # Keep only original and DP columns
    selected_columns = numeric_columns + [f"{col}_DP" for col in numeric_columns]
    df_dp = df_dp[selected_columns]

    # Save and display results
    df_dp.to_csv("dp_vehicle_data.csv", index=False)
