...
#     """
#     Fetches vehicle data from the CSV file with pagination.
#     """
...
#     logging.info(f"LOG: Fetching vehicle data from data file: {data_file} for vehicle ID: {vehicle_id}")
...
#                 if f.startswith("dp_") and f.endswith(".csv")
#             ]
#             file = data_file
#             if matching_files:
...
#     if not os.path.exists(data_file_path):
#         logging.error(f"LOG: !ERROR! Vehicle data not found for data file: {data_file}")
#         return jsonify({"error": "No data available. Run the simulation first."}), 404
