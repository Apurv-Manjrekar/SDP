...
def calculate_risk(df):
    ...
    df["Acceleration_Risk"] = df['FastAccelerations'] * WEIGHTS["acceleration"]
...
if __name__ == "__main__":
    ...
    df = pd.read_csv('/Users/aminsheikh/Desktop/CSE4940/Risk-Score/processed_sumo_features.csv')
    # Load the dataset
    #df = pd.read_csv(results_dir_path + "/" + dataset)

    road_risk_scores = calculate_risk(df)
    road_risk_scores.to_csv("combine_analysis.csv", index=False)
