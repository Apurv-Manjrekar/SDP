...
import os
import argparse
from data_pipeline import process_sumo_data
...
def calculate_risk(df):
    ...
    df["Speeding_Risk"] = (df["SpeedingViolations"] / df["Instance_Count"]) * WEIGHTS["speeding"]
    df["Acceleration_Risk"] = (df["FastAccelerations"] / df["Instance_Count"]) * WEIGHTS["acceleration"]
    df["Braking_Risk"] = (df["HardBrakings"] / df["Instance_Count"]) * WEIGHTS["braking"]
    df["Lane_Change_Risk"] = (df["LaneChanges"] / df["Instance_Count"]) * WEIGHTS["lane_change"]
    df["Headway_Risk"] = (df["UnsafeHeadways"] / df["Instance_Count"]) * WEIGHTS["headway"]
    ...
    road_risk_scores = df[["Vehicle_ID", "Risk_Score"]]
...
if __name__ == "__main__":
    ...
    df = process_sumo_data(dataset_path)
